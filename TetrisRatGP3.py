...
class Rat:
    ...
    def __init__(self, pop, frames_per_second, numTests, mode =0):
        self.numGames = 0
        self.popAvg = 0
        self.goalPos = None
        self.popSize = pop
    ...
    def rewardFunc(self, A):
        ...
        params['B'] = self.bumps(HA)*(self.currentGenome.hyperParams[0])
        params['A'] = self.avgHeight(A,HA)*(self.currentGenome.hyperParams[1])
        params['H'] = self.numHoles(A,HA)*(self.currentGenome.hyperParams[2])
        params['C'] = self.completeLines(A,HA)*(self.currentGenome.hyperParams[3])
        params['L'] = self.boardLevel(A)*(self.currentGenome.hyperParams[4])
        params['M'] = self.maxHeight(HA)*(self.currentGenome.hyperParams[5])
...
def evaluatePopulation(agent, population):
    for gene in population:
        ...
        gene.fitness = avgScore(newLines)#/max(1,avgScore(newLines))
    return population
...
def train(training_name = 'solo', goal=100,tests=16, start_size = 10,lookback_distance=101, impressive_score=1000):
    #temp = 1
    generation = 0
    bestGenome = None
    topScores = []
    log = {}
    agent = Rat(pop=start_size, frames_per_second=150, numTests = tests)
    ...
    while generation < goal:
        ...
        for genome in agent.population:
            ...
            print(x,'\tf:',round(genome.fitness),'\ttag',tagStr,'\tgene',genome)
            x-=1
        print('============================================================================================================================')
        print('|%s:'%training_name,'Generation: ',generation,'\tBest Fitness: ',round(bestGenome.fitness, ndigits=3), '\tAverage Fitness: ', round(aver,ndigits=3),'\tPercent to Goal: ',round((generation/goal)*100, ndigits=2),'%','|')
        print('============================================================================================================================')
...
if __name__ == '__main__':
    ...
    if multPr == True:
        ...
        for x in range(len(processes)):
            processes[x] = Process(target = train, args=('process%i'%x, itterations, 16))
            processes[x].start()
